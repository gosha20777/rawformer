...
from rawformer.torch.select             import select_optimizer, select_loss
from rawformer.torch.image_masking      import select_masking
...
class Autoencoder(ModelBase):
    ...
    def _setup_losses(self, config):
        self.loss_fn = select_loss(config.loss).to(self.device)
