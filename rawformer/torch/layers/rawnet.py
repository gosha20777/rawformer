...
class CQA(nn.Module):
    def __init__(self, dim, num_heads, bias, token_size):
        super(CQA, self).__init__()
        self.num_heads = num_heads
        #self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.temperature = 1 / torch.sqrt(torch.tensor(dim, dtype=torch.float32))

        self.qkv = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=bias)
        self.q_pwconv = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.project_out = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.pool = nn.AvgPool2d(2, 2)
        self.token_size = (token_size, token_size)
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x)
        q, k, v = qkv.chunk(3, dim=1)
        q_reduced = self.q_pwconv(self.pool(q))
        b_r, c_r, h_r, w_r = q_reduced.shape
        N_h, N_w  = calc_tokenized_size((c, h, w), self.token_size)
        N_h_r, N_w_r  = calc_tokenized_size((c_r, h_r, w_r), self.token_size)


        q = img_to_tokens(q, self.token_size)
        k = img_to_tokens(k, self.token_size)
        v = img_to_tokens(v, self.token_size)
        q_reduced = img_to_tokens(q_reduced, self.token_size)
        q = q.reshape((q.shape[0], N_h * N_w, -1))
        k = k.reshape((k.shape[0], N_h * N_w, -1))
        v = v.reshape((v.shape[0], N_h * N_w, -1))
        q_reduced = q_reduced.reshape((q_reduced.shape[0], N_h_r * N_w_r, -1))
        

        q = torch.nn.functional.normalize(q, dim=-1)
        q_reduced = torch.nn.functional.normalize(q_reduced, dim=-1)
        k = torch.nn.functional.normalize(k, dim=-1)

        
        attn_h = torch.matmul(q_reduced, k.transpose(-1, -2)) * self.temperature
        attn_h = attn_h.softmax(dim=-1)
        attn_u = torch.matmul(q, q_reduced.transpose(-1, -2)) * self.temperature
        attn_u = attn_u.softmax(dim=-1)
        
        out = torch.matmul(attn_h, v)
        out = torch.matmul(attn_u, out)
        out = out.reshape((
            out.shape[0], N_h, N_w, c, *self.token_size
        ))
        out = img_from_tokens(out)
        out = self.project_out(out)
        return out
...
class ModCQA(nn.Module):
    def __init__(self, dim, num_heads, bias, token_size,
                 eps = 1e-6, demod = True):
        super(ModCQA, self).__init__()
        self.num_heads = num_heads
        #self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.temperature = 1 / torch.sqrt(torch.tensor(dim, dtype=torch.float32))
    ...
    def forward(self, x, s):
        ...
        qkv = self.qkv(x)
        q, k, v = qkv.chunk(3, dim=1)
        q_mod = q * s.unsqueeze(2).unsqueeze(3)
        q_reduced = self.q_pwconv(self.pool(q_mod))
        b_r, c_r, h_r, w_r = q_reduced.shape
        N_h, N_w  = calc_tokenized_size((c, h, w), self.token_size)
        N_h_r, N_w_r  = calc_tokenized_size((c_r, h_r, w_r), self.token_size)


        q_mod = img_to_tokens(q_mod, self.token_size)
        k = img_to_tokens(k, self.token_size)
        v = img_to_tokens(v, self.token_size)
        q_reduced = img_to_tokens(q_reduced, self.token_size)
        q_mod = q_mod.reshape((q_mod.shape[0], N_h * N_w, -1))
        k = k.reshape((k.shape[0], N_h * N_w, -1))
        v = v.reshape((v.shape[0], N_h * N_w, -1))
        q_reduced = q_reduced.reshape((q_reduced.shape[0], N_h_r * N_w_r, -1))
        
        q_mod = torch.nn.functional.normalize(q_mod, dim=-1)
        q_reduced = torch.nn.functional.normalize(q_reduced, dim=-1)
        k = torch.nn.functional.normalize(k, dim=-1)

        attn_h = torch.matmul(q_reduced, k.transpose(-1, -2)) * self.temperature
        attn_h = attn_h.softmax(dim=-1)
        attn_u = torch.matmul(q_mod, q_reduced.transpose(-1, -2)) * self.temperature
        attn_u = attn_u.softmax(dim=-1)
...
class SPFN(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.ReLU, drop=0.):
        ...
        self.pointwise_3x3_1 = torch.nn.Conv2d(in_features, hidden_features, kernel_size=1)
        self.depthwise_3x3 = torch.nn.Conv2d(hidden_features,hidden_features, kernel_size=3,stride=1,padding=1,dilation=1,groups=hidden_features)
        self.pointwise_3x3_2 = torch.nn.Conv2d(hidden_features*2, out_features, kernel_size=1)
        
        self.pointwise_5x5_1 = torch.nn.Conv2d(in_features, hidden_features, kernel_size=1)
        self.depthwise_5x5 = torch.nn.Conv2d(hidden_features, hidden_features, kernel_size=5,stride=1, padding=2, dilation=1, groups=hidden_features)
        self.pointwise_5x5_2 = torch.nn.Conv2d(hidden_features*2, out_features, kernel_size=1)
        
        self.act_layer = act_layer() if act_layer is not None else nn.Identity()
    def forward(self, x):
        x_3x3 = self.pointwise_3x3_1(x)
        x_3x3 = self.depthwise_3x3(x_3x3)
        x_3x3 = self.act_layer(x_3x3)
        x_5x5 = self.pointwise_5x5_1(x)
        x_5x5 = self.depthwise_5x5(x_5x5)
        x_5x5 = self.act_layer(x_5x5)

        x_3x3_c = torch.cat([x_5x5, x_3x3], dim=1)
        x_3x3_c = self.pointwise_3x3_2(x_3x3_c)
        x_5x5_c = torch.cat([x_3x3, x_5x5], dim=1)
        x_5x5_c = self.pointwise_5x5_2(x_5x5_c)
        x = x_3x3_c + x_5x5_c
        return x
...
class RawFormerBlock(nn.Module):
    def __init__(
        ...
        num_heads = 1, token_size = 4, **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self.attn = CQA(in_features, num_heads, token_size=token_size, bias=True)
        ...
        self.ffn = SPFN(in_features, mid_features, in_features)
...
class ModRawFormerBlock(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.norm1 = get_norm_layer(norm, in_features)
        self.attn = ModCQA(in_features, num_heads, token_size=token_size, bias=True, demod=demod)
        self.dw = torch.nn.Conv2d(in_features, in_features, kernel_size=3,stride=1,padding=1,dilation=1,groups=in_features)
        self.pw = torch.nn.Conv2d(in_features*2, in_features, kernel_size=1)
        self.norm2 = get_norm_layer(norm, in_features)
        self.ffn = SPFN(in_features, mid_features, in_features)
...
class DBlock(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        (C, H, W)  = input_shape
        self.block = RawFormerBlock(C, features, activ, norm, num_heads=num_heads, token_size=token_size)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conv = DBlock(
            ...
            num_heads = num_heads, token_size = token_size
        )
